...
def register_verify_otp(payload: VerifyOtpSchema, db: Session = Depends(get_db)):
    ...
    # stored_data IS the user info, no ["user_info"] key
    ...
    # --- FIX 1: Removed user_id and phone from SQLAlchemy Model instantiation ---
    # Assuming your model column is named 'id'. If it's named 'user_id' but autoincrements, 
    # just omit it completely here so the DB handles it.
    new_user = User(
    ...
    db.add(new_user)
    db.commit()
    db.refresh(new_user)  # This fetches the newly generated auto-incremented ID from the DB
    
    res.delete(payload.email)

    # --- FIX 2: Removed phone from event_data ---
    event_data = {
    ...
    # Send to 'user_events' (Router listens here)
    producer.send('user_events', value=event_data)
    producer.flush()

    # --- FIX 3: Pass the auto-incremented DB ID to Neo4j Kafka event ---
    kafka_event = {
        "action": "create_user",
        "username": f"{new_user_info['firstname']} {new_user_info['lastname']}",
        "email": new_user_info["email"],
        "user_id": new_user.id  # <-- Uses the ID generated by your SQL database
    }
